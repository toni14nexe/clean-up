  ...
  if (event.req.method === 'POST') {
    ...
    const orderItemsHtml = order.list
      ...
      <div style="margin-bottom: 10px; padding: 8px; border-bottom: 1px solid #eee;">
        ...
        ${item.busLine ? '<p>Autobusna linija: ' + item.busLine + '</p>' : ''}
        ${item.childClothes ? '<p>Dječje: Da' : ''}
        ${item.unisex ? '<p>Spol: ' + 'Unisex</p>' : ''}
        ${item.menWomenOption ? '<p>Spol: ' + item.menWomenOption + '</p>' : ''}
        ${item.blackWhiteOption ? '<p>Boja: ' + item.blackWhiteOption + '</p>' : ''}
        ${item.size ? '<p>Veličina: ' + item.size + '</p>' : ''}</p>
        <p>Cijena: ${item.price.toFixed(2)} eura</p>
        <p>Ukupno: ${item.totalPrice.toFixed(2)} eura</p>
      </div>
    `
      )
      .join('')
    ...
    const mailOptions = {
      ...
      subject: `s.art narudžba ${order.orderNumber}`,
      html: `
               ...
               <p>Email: info@sart.hr</p>
               <p>Mobitel: +385 99 7900 257</p>
               <p>Adresa: Baćina 2</p>
               <p>Mjesto: 51219 Čavle</p>
               <p>Država: Hrvatska</p>
               <br/>
               <b><p>Dostava: ${order.shipping}${body.shippingPrice ? ` - ${body.shippingPrice.toFixed(2)} eura` : ''}</p></b>
               <b><p>Plaćanje: ${order.payment}</p></b>
               <br/>
               <b><p>Narudžba</p></b>
               ${orderItemsHtml}
               <b><p>Ukupna cijena: ${body.totalPrice.toFixed(2)} eura</p></b>
               <br/>
