...
const form = reactive<RuleForm>({
  ...
  childClothesType: '',
  unisex: false,
  menWomenOption: false,
  blackWhiteOption: false,
  price: 0,
...
const rules = reactive<FormRules<RuleForm>>({
  ...
  discountPrice: [
    {
      required: form.discountPrice ? true : false,
      message: 'Unesite akcijsku cijenu proizvoda',
      trigger: 'blur'
    },
    {
      validator: (_, value, callback) => {
        if (form.discountPrice) {
          if (isNaN(value)) {
            return callback(new Error('Unesite ispravnu brojčanu vrijednost'))
          }
          if (Number(value) >= Number(form.price)) {
            return callback(
              new Error('Akcijska cijena mora biti manja od redovne cijene')
            )
          }
        }
        return callback()
...
function openDialog(type: DialogType, item?: Product) {
  if (item) {
    ...
    form.sizeOption = Boolean(item.sizeOption)
    form.childClothes = Boolean(item.childClothes)
    form.childClothesType = String(item.childClothesType)
    form.unisex = Boolean(item.unisex)
    form.menWomenOption = Boolean(item.menWomenOption)
    form.blackWhiteOption = Boolean(item.blackWhiteOption)
...
function handleCloseDialog() {
  ...
  form.description = ''
  form.price = 0
  form.sizeOption = true
  form.childClothes = false
  form.childClothesType = 'tshirt'
  form.unisex = false
  form.menWomenOption = false
  form.blackWhiteOption = false
...
function handleChildClothesChange(value: string | number | boolean) {
  if (!value) form.childClothesType = ''
  else form.childClothesType = 'tshirt'
}
...
async function handleCreate(formEl: FormInstance | undefined) {
  ...
  await formEl.validate(async valid => {
    if (valid) {
      ...
      if (!form.discountPriceSwitch) form.discountPrice = undefined
      try {
  ...
  >
    ...
    >
      ...
      >
        ...
        <ElFormItem label="Izbor veličina" class="mt-20">
          <ElSwitch
            v-model="form.sizeOption"
            size="large"
            active-text="Uključi"
            inactive-text="Isključi"
          />
        </ElFormItem>
        <ElFormItem label="Izbor crno-bijelo" class="mt-20">
          <ElSwitch
            v-model="form.blackWhiteOption"
            size="large"
            active-text="Uključi"
            inactive-text="Isključi"
          />
        ...
        <ElFormItem
          v-if="!form.menWomenOption && !form.unisex"
          label="Dječji proizvod"
          class="mt-20"
        >
          <ElSwitch
            v-model="form.childClothes"
            size="large"
            active-text="Uključi"
            inactive-text="Isključi"
            @change="handleChildClothesChange"
          />
        </ElFormItem>
        <ElFormItem v-if="form.childClothes" label="Dječji tip" class="mt-20">
          <ElRadioGroup v-model="form.childClothesType">
            <ElRadio value="tshirt">Kratka majica</ElRadio>
            <ElRadio value="shirt">Duga majica</ElRadio>
          </ElRadioGroup>
        </ElFormItem>
        <ElFormItem
          v-if="!form.menWomenOption && !form.childClothes"
          label="Unisex"
          class="mt-20"
        >
          <ElSwitch
        ...
        <ElFormItem
          v-if="!form.unisex && !form.childClothes"
          label="Muško-žensko"
          class="mt-20"
        >
          <ElSwitch
            v-model="form.menWomenOption"
            size="large"
            active-text="Uključi"
            inactive-text="Isključi"
          />
        ...
        <ElFormItem label="Akcijska cijena" prop="discountPrice" class="mt-20">
          <ElInputNumber
            ...
            :step="0.01"
            class="input-width"
            :disabled="!form.discountPriceSwitch"
          />
        </ElFormItem>
