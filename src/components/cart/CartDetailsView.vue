<script setup lang="ts">
const emits = defineEmits(['nextStep'])
const router = useRouter()
const cart = ref(getCartItems())
const cartStore = useCartStore()
const { cartChangeState } = storeToRefs(cartStore)

function handleQuantityChange(product: CartItem) {
  if (product.quantity === 0) removeFromCart(product.id)
  else cart.value = updateQuantity(product.id, Number(product.quantity))
}

function handleNextStep() {
  cart.value = cart.value.filter(p => p.quantity > 0)
  localStorage.setItem('cart', JSON.stringify(cart.value))

  emits('nextStep')
}

function aaa() {
  clearCart()
  router.push('/webshop')
}
</script>

<template>
  <ElRow align="middle" justify="center">
    <h3 class="color-primary">Košarica</h3>
  </ElRow>

  <ElEmpty v-if="!getCartTotal()" description="Košarica je prazna" />

  <template v-else>
    <ElRow justify="center" align="middle" class="mb-16">
      <ElCol :xs="0" :sm="4" />
      <ElCol :xs="9" :sm="8">
        <p>NAZIV</p>
      </ElCol>
      <ElCol :xs="3" :sm="3">
        <p>INFO</p>
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <p>CIJENA</p>
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <p>{{ $viewport.isGreaterOrEquals('tablet') ? 'KOLIČINA' : 'KOL' }}</p>
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <p>UKUPNO</p>
      </ElCol>
    </ElRow>
    <ElDivider />

    <ElRow v-for="product in cart" justify="center" align="middle">
      <ElCol
        v-if="$viewport.isGreaterOrEquals('tablet')"
        :xs="0"
        :sm="4"
        align="middle"
      >
        <img v-if="product?.image" :src="product.image" class="cart-image" />
        <ElEmpty v-else class="cart-image" description="Nema slike" />
      </ElCol>
      <ElCol :xs="9" :sm="8">
        <p>{{ product.name }}</p>
      </ElCol>
      <ElCol :xs="3" :sm="3">
        <ElTooltip effect="dark" placement="top-start" trigger="click">
          <ElIcon class="tooltip-triger"><ElIconInfoFilled /></ElIcon>
          <template #content>
            <template v-if="product.busLine">
              <span class="tooltip-content-text">
                Linija:
                {{ product.busLine }}
              </span>
              <br />
            </template>
            <template v-if="product.menWomenOption">
              <span class="tooltip-content-text">
                Spol:
                {{ product.menWomenOption === 'men' ? 'Muško' : 'Žensko' }}
              </span>
              <br />
            </template>
            <template v-if="product.blackWhiteOption">
              <span class="tooltip-content-text">
                Boja:
                {{ product.blackWhiteOption === 'black' ? 'Crna' : 'Bijela' }}
              </span>
              <br />
            </template>
            <span v-if="product.size" class="tooltip-content-text">
              Veličina: {{ product.size }}
            </span>
          </template>
        </ElTooltip>
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <p>{{ `${product.price.toFixed(2)}` }}</p>
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <ElInput
          v-model="product.quantity"
          type="number"
          :min="0"
          @change="handleQuantityChange(product)"
          @input="handleQuantityChange(product)"
          size="small"
          :style="`width: ${$viewport.isGreaterOrEquals('tablet') ? '60px' : '43px'};`"
        />
      </ElCol>
      <ElCol :xs="4" :sm="3">
        <p>{{ `${product.totalPrice.toFixed(2)}` }}</p>
      </ElCol>
      <ElDivider />
    </ElRow>

    <ElRow justify="center" align="middle" class="mt-16">
      <ElCol
        :xs="5"
        :sm="4"
        :offset="$viewport.isGreaterOrEquals('tablet') ? 20 : 19"
      >
        <p :key="cartChangeState">{{ `${getCartTotal().toFixed(2)}` }}</p>
      </ElCol>
    </ElRow>

    <ElRow justify="space-around" align="middle" class="mt-8">
      <ElButton type="danger" @click="aaa" plain>Isprazni košaricu</ElButton>
      <ElButton type="primary" @click="handleNextStep" plain>Nastavi</ElButton>
    </ElRow>
  </template>
</template>

<style scoped>
.cart-image {
  width: 96px;
  height: 96px;
}

.tooltip-triger {
  color: var(--el-color-primary);
  cursor: pointer;
  transition: 0.2s ease-in-out;
}

.tooltip-triger:hover {
  color: var(--el-color-primary-light-3);
  transition: 0.2s ease-in-out;
}

.tooltip-content-text {
  font-size: 18px;
}

@media (max-width: 767px) {
  p {
    font-size: small;
    font-weight: 600;
  }
}

@media (max-width: 350px) {
  p {
    font-size: x-small;
  }
}
</style>
